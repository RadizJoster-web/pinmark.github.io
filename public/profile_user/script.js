...
const disimpan = document.getElementById("disimpan");
const formDisimpan = document.getElementById("formDisimpan");

dibuat.addEventListener("change", function () {
  if (dibuat.checked === true) {
    formUpload.classList.add("active");
    disimpan.checked = false; // pastikan checkbox 'disimpan' tidak aktif
    formDisimpan.classList.remove("active");
  } else {
    formUpload.classList.remove("active");
  }
});

disimpan.addEventListener("change", async function () {
  if (disimpan.checked === true) {
    formDisimpan.classList.add("active");
    dibuat.checked = false; // pastikan checkbox 'dibuat' tidak aktif
    formUpload.classList.remove("active");
  } else {
    formDisimpan.classList.remove("active");
  }
...
// ----------------------------------
// Form untuk Gambar yang dibuat
// ----------------------------------
const gallery = document.querySelector(".gallery");
window.addEventListener("DOMContentLoaded", async () => {
  const username = window.location.pathname.split("/")[2];
  try {
    const res = await fetch(`/${username}/gallery`); // Pastikan endpoint ini mengembalikan JSON yang benar
    if (!res.ok) {
    ...
    if (result && result.length > 0) {
      ...
      images.forEach((image) => {
        image.addEventListener("click", async function () {
          ...
          if (showimg.profileUser.length > 0) {
            const imgProfile = document.createElement("img");
            imgProfile.src = showimg.profileUser;
          ...
          h1.textContent = showimg.username;
          ...
          const p = document.createElement("p");
          p.textContent = showimg.titleImage;
...
// ----------------------------------
// Form untuk Gambar yang disimpan
// ----------------------------------
const galleryDisimpan = document.querySelector(".galleryDisimpan");
window.addEventListener("DOMContentLoaded", async () => {
  const username = window.location.pathname.split("/")[2];
  try {
    const res = await fetch(`/${username}/borkmark`); // Pastikan endpoint ini mengembalikan JSON yang benar
    if (!res.ok) {
      console.error("Gagal ambil image");
      return;
    }
    ...
    if (result && result.length > 0) {
      const columns = galleryDisimpan.querySelectorAll(".column");
      ...
      const images = galleryDisimpan.querySelectorAll("img");
      const zoomImg = document.getElementById("zoomImg");
      ...
      images.forEach((image) => {
        image.addEventListener("click", async function () {
          ...
          h1.href = `/profile/${showimg.username}`;
          h1.textContent = showimg.username;
